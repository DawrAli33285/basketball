...
offerid.push(val?.logoid)
  ...
  // if(logoUrls[0])offers.logo=logoUrls[0]
      ...
      if (profile) {
        ...
        if(previousCoachName)playerUpdateFields.previousCoachName=previousCoachName;
  ...
  if(coach.type)data.type=coach.type
          ...
          let updatedCoach=await coachModel.updateOne({ auth: id }, { $set: data });
  ...
  module.exports.getPlayers = async (req, res) => {
    ...
    try {
      ...
      let players = await playerModel.find({}).populate('auth').populate('institute');
